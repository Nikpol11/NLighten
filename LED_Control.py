...
class LED_Control:
    ...
    def set_white_balance(self, value):
        self.white_balance = max(0.0, min(1.0, float(value)))
        
    def runCurrentMode(self):
        self.ldrAverage.append(self.ldr.value)
        ...
        if self.ldrAverageVal <= self.threshold[0] and self.brightness != 0:
            if self.led.brightness == 0: self.onTime = time.time()
            self.led.brightness = (1/99)*((100**self.brightness)-1)
            self.on = True
    ...
    def allWhite(self):
        ...
        for i in range(7):
            self.led[i] = (rgb_intensity, rgb_intensity, rgb_intensity, w_intensity)
    
    def singleWhite(self):
        for i in range(7):
            self.led[i] = (0,0,0,255)

    def rgbWhite(self):
        for i in range(7):
            self.led[i] = (255,255,255,0)

    def singleAllWhite(self):
        rgb_intensity = int(255 * (1.0 - self.white_balance))
        w_intensity = int(255 * self.white_balance)
        if rgb_intensity == w_intensity: rgb_intensity, w_intensity = (255,255)
        self.led[0] = (rgb_intensity, rgb_intensity, rgb_intensity, w_intensity)
    ...
    def basicTest(self):
        for i in range(7):
            self.led[i] = (0,255,0,0)
            time.sleep(0.5)
            self.led[i] = (255,0,0,0)
            time.sleep(0.5)
            self.led[i] = (0,0,255,0)
            time.sleep(0.5)
            self.led[i] = (0,0,0,255)
            time.sleep(0.5)
        
    # effects below made with Claude 3.7 and later models
    ...
    def breathingEffect(self):
        ...
        for i in range(7):
            self.led[i] = (int(r * breath_value), int(g * breath_value), int(b * breath_value), 0)
    ...
    def sparkleEffect(self):
        ...
        for i in range(7):
            r, g, b, w = self.led[i]
            self.led[i] = (max(0, int(r*0.94)), max(0, int(g*0.94)), max(0, int(b*0.94)), max(0, int(w*0.94)))
    ...
    def colorWipeEffect(self):
        ...
        for i in range(7):
            ...
            if position <= 2:  # Length of trail (0 = active LED, 1 & 2 = trailing LEDs)
                ...
                self.led[i] = (int(r * intensity), int(g * intensity), int(b * intensity), 0)
            else:
                self.led[i] = (0, 0, 0, 0)  # Off
    ...
    def pulseEffect(self):
        ...
        self.led[0] = (int(r * center_intensity), int(g * center_intensity), int(b * center_intensity), 0)
    ...
    def starburstEffect(self):
        ...
        if intensity > 0:
            ...
            for i in range(7):
                self.led[i] = (int(r * intensity), int(g * intensity), int(b * intensity), int(255 * intensity / 3))
        else:
    ...
    def meteorEffect(self):
        ...
        for i in range(7):
            r, g, b, w = self.led[i]
            self.led[i] = (max(0, int(r*0.95)), max(0, int(g*0.95)), max(0, int(b*0.95)), max(0, int(w*0.95)))
    ...
    def nightSkyEffect(self):
        ...
        if self.counter % 250 == 0 and len([False for i in range(7) if self.led[i][3] > 0]) == 0 : update_led = random.randint(0,6)
        else: update_led = None
        
        # Deep blue base for all LEDs (night sky)
        for i in range(7):
            # Get current values
            current_r, current_g, current_b, current_w = self.led[i]
    ...
    def heartbeatEffect(self):
        ...
        for i in range(7):
            ...
            # Outer LEDs at 70% intensity
                ...
                self.led[i] = (int(r * 0.7), int(g * 0.7), int(b * 0.7), 0)
